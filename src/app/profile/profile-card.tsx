...
const KidCard = ({ kid }: KidCardProps) => {
    ...
    return (
		<>
			{openViewDialog && <ViewDialog kid={kid} openDialog={openViewDialog} setOpenDialog={setOpenViewDialog} />}
				...
				<CardHeader className="relative flex w-full flex-col items-center justify-center">
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button className="absolute right-0 top-0 size-8" size="icon" variant="secondary">
						...
						<DropdownMenuContent>
							...
							<DropdownMenuItem onClick={() => setOpenViewDialog(!openViewDialog)}>
								<Eye size={16} />
								View
							</DropdownMenuItem>
							<DropdownMenuItem onClick={() => setOpenUpdateDialog(!openUpdateDialog)}>
								<Edit size={16} />
								Update
							</DropdownMenuItem>
						</DropdownMenuContent>
					...
					<Avatar className="size-24 border-2 border-secondary p-0.5">
						...
						<AvatarFallback className="text-4xl font-light text-secondary">{kid?.firstName?.at(0)}</AvatarFallback>
					</Avatar>
				</CardHeader>
				<CardContent className="flex w-full flex-col justify-stretch">
					<div className="flex w-full gap-2">
						...
						<div className="flex w-full flex-col">
							...
							<p className="break-all text-sm font-medium">{kid?.middleName ? `${kid?.title} ${kid?.firstName} ${kid?.middleName} ${kid?.lastName}` : `${kid?.title} ${kid?.firstName} ${kid?.lastName}`}</p>
						</div>
					</div>
					<div className="flex w-full gap-2">
						...
						<div className="flex w-full flex-col">
							...
							<p className="break-all text-sm font-medium">{kid?.user?.email}</p>
						</div>
					</div>
					<div className="flex w-full gap-2">
						...
						<div className="flex w-full flex-col">
							...
							<p className="break-all text-sm font-medium">{format(parse(kid?.dateOfBirth, "yyyy-MM-dd", new Date()), "MMM do, yyyy")}</p>
						</div>
					</div>
				</CardContent>
			</Card>
