...
const ViewDialog = ({ openDialog, setOpenDialog, kid }) => {
  return (
    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
      <DialogContent
        className="max-w-xl"
        onOpenAutoFocus={(event) => event.preventDefault()}
        aria-labelledby="view-kid-title"
        aria-describedby="view-kid-desc"
      >
        <DialogHeader>
          <DialogTitle id="view-kid-title">View Kid</DialogTitle>
          <DialogDescription id="view-kid-desc">View kid details</DialogDescription>
        </DialogHeader>
        <ScrollArea className="max-h-96 w-full px-2">
          <div className="xs:grid-cols-2 grid grid-cols-1 gap-2">
            <Avatar className="border-secondary xs:col-span-2 my-2 size-24 self-center justify-self-center border-2 p-0.5">
              ...
              />
              <AvatarFallback className="text-secondary text-4xl font-light">
            ...
            {kid?.dateOfBirth && (
              <div className="flex w-full gap-2">
                ...
                <div className="flex w-full flex-col">
                  ...
                  <p className="text-sm font-medium break-all">
                    {format(parse(kid.dateOfBirth, "yyyy-MM-dd", new Date()), "MMM do, yyyy")}
                  </p>
                </div>
              </div>
            )}
            ...
            {kid?.address?.addressLine1 && (
              <div className="flex w-full gap-2">
                ...
                <div className="flex w-full flex-col">
                  ...
                  <p className="text-sm font-medium break-all">
                    {kid.address?.addressLine2
                      ? `${kid.address.addressLine1}, ${kid.address.addressLine2}`
                      : kid.address.addressLine1}
                  </p>
                </div>
              </div>
            ...
            <>
              ...
              <div className="flex w-full flex-col justify-center gap-2">
                ...
                <div className="flex w-full flex-wrap items-center gap-2">
                  {kid.languages.map((language, index) => (
                ...
                <div className="xs:grid-cols-2 grid grid-cols-1 gap-2">
                  {kid.guardians.map((guardian) => (
                    <div
                      ...
                      key={guardian?.user?.uuid}
                    >
                      <div className="relative p-2">
                        {guardian?.guardianType === "PRIMARY" && (
                          <Crown
                            className="fill-secondary stroke-secondary absolute top-0 left-0 z-10 -rotate-45"
                            size={16}
                          />
                        )}
                        <Avatar className="size-14 rounded-xl">
                          <AvatarImage
                            ...
                            src={guardian?.user?.avatarUrl || undefined}
                          />
                      ...
                      <div className="flex w-full flex-col items-start justify-center gap-0.5">
                        ...
                        <p className="text-muted-foreground line-clamp-1 text-left text-xs break-all">
                          {guardian?.user?.email}
                        </p>
                  ...
                  {kid.organizations.map((organization) => (
                    ...
                    >
                      ...
                      <div className="flex w-full flex-col items-start justify-center">
                        ...
                        <p className="text-muted-foreground line-clamp-1 text-left text-xs break-all">
                          {organization.identificationNo || "N.A"}
                        </p>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div
                                className="flex size-12 shrink-0 items-center justify-center gap-1 rounded-xl"
                                aria-label="Approved"
                                title="Approved"
                              >
