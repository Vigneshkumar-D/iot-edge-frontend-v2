...
const AuthPlaceholder = () => {
  ...
  const renderImage = (src, alt) => (
    <section className="sticky inset-y-0 left-0 hidden max-h-screen w-1/3 xl:block">
      <img alt={alt} className="h-screen w-full object-cover" src={src} />
    </section>
  );

  if (path === "/auth/register") {
    if (type === "INDIVIDUAL") {
      return renderImage(registerIndividualPlaceholder, "register-individual-placeholder");
    } else if (type === "ORGANIZATION") {
      return renderImage(registerOrganizationPlaceholder, "register-organization-placeholder");
    }
  }

  return renderImage(authPlaceholder, "auth-placeholder");
};
