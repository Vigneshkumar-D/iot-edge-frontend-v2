    ...
    <DayPicker
      ...
      components={{
        ...
        MonthGrid: ({ className, children, ...monthGridProps }) => {
          ...
          if (navView === "years") {
            return (
              <div className={cn("grid grid-cols-4", className)} {...monthGridProps}>
                {Array.from({ length: years.to - years.from + 1 }, (_, index) => {
                  const year = years.from + index;
                  const isBefore =
                    differenceInCalendarDays(new Date(year, 12, 31), startMonth) < 0;
                  const isAfter =
        ...
        Nav: ({ className }) => {
          ...
          const yearSpan = years.to - years.from + 1;
          const isPreviousDisabled =
            navView === "years"
              ? (startMonth &&
          ...
          const handlePreviousClick = () => {
            if (navView === "years") {
              ...
              onPrevClick?.(new Date(years.from - yearSpan, 0, 1));
          ...
          const handleNextClick = () => {
            if (navView === "years") {
              ...
              onNextClick?.(new Date(years.from + yearSpan, 0, 1));
            } else if (nextMonth) {
              goToMonth(nextMonth);
              onNextClick?.(nextMonth);
            }
