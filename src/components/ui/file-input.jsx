...
const FileInput = forwardRef(
  (
    ...
    const onDrop = async (acceptedFiles, rejectedFiles) => {
      if (rejectedFiles.length > maxFiles || files.length + acceptedFiles.length > maxFiles) {
        toast.error("Error!", {
      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ file }) => {
          toast.error("Error!", {
            description: `Cannot select ${file.name}!`,
          });
        });
      }
                ...
                <h1 className="text-foreground text-sm font-medium">
                  {isDragActive
                    ...
                    : `Drop your ${
                      ...
                      } here or click to select your ${maxFiles > 1 ? "files" : "file"}`}
                </h1>
                <p className="text-muted-foreground text-xs sm:text-sm">
                  You can upload{" "}
                  {maxFiles > 1
                    ? `${maxFiles === Infinity ? "multiple" : maxFiles} files (up to ${formatBytes(
                        maxSize
                      )} each)`
        ...
        {files.length > 0 && (
          <ScrollArea className="h-fit w-full">
            <div className="max-h-40 space-y-4">
              {files.map((file, index) => (
                <div
                  className="flex w-full items-center justify-between gap-2 bg-input rounded-xl p-2"
                  key={index}
                >
                  <File className="stroke-1" size={36} />
                  <div className="flex w-full flex-col justify-center overflow-hidden">
