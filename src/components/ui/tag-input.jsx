...
export const TagInput = ({
  ...
  const onRemove = (val) => {
    if (selectedValues.includes(val)) {
      const updated = selectedValues.filter((item) => item !== val);
  ...
  const onPaste = (event) => {
    ...
    tags.forEach((item) => {
      const parsedItem = item.replaceAll(FORMATTING_REGEX, "").trim();
      if (parsedItem && !newValue.includes(parsedItem)) {
        newValue.push(parsedItem);
      }
    });
    setSelectedValues(newValue);
  ...
  const onKeyDown = (event) => {
    ...
    switch (event.key) {
      ...
      case "Escape":
        setActiveIndex(
          activeIndex === -1 ? selectedValues.length - 1 : -1
        );
        break;
    }
  };
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge
                  aria-disabled={disabled}
                  className={cn(
                    "relative flex max-w-56 cursor-pointer items-center gap-1 rounded px-1 text-xs",
                    activeIndex === index &&
                      "ring-2 ring-ring data-[active='true']"
                  )}
                  data-active={activeIndex === index}
                  tabIndex={activeIndex !== -1 ? 0 : activeIndex}
                  variant={variant}
